...
function setAttributeYouTubeFrame(youTubeKey){
    ...
    youTubePlayer.setAttribute('src', `https://www.youtube.com/embed/${youTubeKey}`); 
}

async function fetchGetMovieTreiler(MOVIE_ID) {
    const { data } = await axios.get(`/movie/${MOVIE_ID}/videos?api_key=${API_KEY}`);
    return data.results;
  }
